...
passport.use('local.registro', new LocalStrategy({
  ...
  passReqToCallback: true
    ...
    const { nombre, telefono, direccion, dni, confirmarContrasena } = req.body;
    let usuarioNuevo = {
      mail, 
      contrasena,
      nombre,
    ...
    if (confirmarContrasena === contrasena) {
      ...
      req.flash('success', 'Registro exitoso. Bienvenido/a ' + usuarioNuevo.nombre)
      return done(null, usuarioNuevo);

    } else {
      return done(null, false, req.flash('message', 'Las contraseñas no son iguales'));
    }
    }));
